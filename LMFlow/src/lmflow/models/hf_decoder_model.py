...
class HFDecoderModel(DecoderModel, Tunable):
        ...
        **kwargs
        ...
        if tune_strategy == 'normal':
            ...
            if model_args.use_lora:
                ...
                model = get_peft_model(model, peft_config)
                model.print_trainable_parameters()
            elif model_args.use_moslora:
                model_args.use_moslora = True
                if model_args.lora_target_modules:
                    lora_target_modules = model_args.lora_target_modules
                else:
                    lora_target_modules = None
                peft_config = LoraConfig(
                    ...
                    lora_dropout=model_args.lora_dropout,
                    target_modules=["q_proj", "o_proj", "v_proj", "k_proj", "gate_proj", "up_proj", "down_proj"],
                    use_moslora=model_args.use_moslora,
                )
                model = get_peft_model(model, peft_config)
                model.print_trainable_parameters()
            elif model_args.use_paca:
                if model_args.lora_target_modules:
                    lora_target_modules = model_args.lora_target_modules
                else:
                    lora_target_modules = None
                peft_config = PacaConfig(
                    ...
                    inference_mode=False,
                    r=model_args.lora_r,
                    paca_alpha=model_args.lora_alpha,
                    gradient_accumulation_steps=model_args.grad_accum_steps,
                    target_modules=["q_proj", "o_proj", "v_proj", "k_proj", "gate_proj", "up_proj", "down_proj"],
                )
                model = get_peft_model(model, peft_config)
                model.print_trainable_parameters()
    ...
    def tokenize(self, dataset, add_special_tokens=True, *args, **kwargs):
        ...
        if model_args.use_lora or model_args.use_paca or data_args.disable_group_texts:
            use_truncation = True
    ...
    def merge_lora_weights(self):
        if (self.model_args.use_lora or self.model_args.use_paca) and not self.model_args.use_qlora:
    ...
    def save(self, dir, save_full_model=False, *args, **kwargs):
        ...
        if save_full_model and (self.model_args.use_lora or self.model_args.use_paca):
            save_dtype = (
                torch.float16
                if self.model_args.torch_dtype in ["auto", None]
                else getattr(torch, self.model_args.torch_dtype)
